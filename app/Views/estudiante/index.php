...
<body class="bg-light">
    ...
    <div class="container mt-4">
        ...
        <!-- Encabezado con titulo, buscador por cedula y boton de nuevo -->
        <div class="row align-items-center mb-3">
            <div class="col-md-3">
                <h3 class="mb-0">Estudiantes</h3>
            </div>

            <!-- Formulario de busqueda por cedula (envio GET tradicional) -->
            <div class="col-md-6">
                <form class="d-flex gap-2" method="GET" action="<?= BASE_URL ?>estudiante/index">
                    <input type="text"
                           class="form-control"
                           name="busqueda"
                    ...
                    <?php if ($busqueda !== ''): ?>
                        <!-- Boton para limpiar la busqueda y ver todo de nuevo -->
                        <a href="<?= BASE_URL ?>estudiante/index" class="btn btn-outline-secondary text-nowrap">
                            Limpiar
                        </a>
                    <?php endif; ?>
                </form>
        ...
        <!-- Resumen de resultados y pagina actual -->
        <div class="d-flex justify-content-between mb-2">
            <small class="text-muted">
                <?= $totalEstudiantes ?> estudiante(s) encontrado(s)
                <?php if ($busqueda !== ''): ?>
                    para la cedula "<?= htmlspecialchars($busqueda) ?>"
                <?php endif; ?>
            </small>
            <small class="text-muted">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                            ...
                            <?php if (empty($estudiantes)): ?>
                                <tr>
                                    <td colspan="9" class="text-center text-muted py-4">
                                        <?php if ($busqueda !== ''): ?>
                                            No se encontraron estudiantes con esa cedula
                                        <?php else: ?>
                ...
                <!-- Paginador: solo se muestra si hay mas de una pagina -->
                <?php if ($totalPaginas > 1): ?>
                    <nav aria-label="Paginacion de estudiantes" class="mt-3">
                        <ul class="pagination justify-content-center mb-0">
                            <!-- Boton anterior -->
                            <li class="page-item <?= $pagina <= 1 ? 'disabled' : '' ?>">
                                <a class="page-link"
                                   href="<?= BASE_URL ?>estudiante/index?pagina=<?= $pagina - 1 ?>&busqueda=<?= urlencode($busqueda) ?>">
                                    Anterior
                                </a>
                            </li>

                            <?php if ($totalPaginas <= 7): ?>
                                <!-- Numeros de pagina cuando hay pocas paginas -->
                                <?php for ($i = 1; $i <= $totalPaginas; $i++): ?>
                                    <li class="page-item <?= $i === $pagina ? 'active' : '' ?>">
                                        <a class="page-link"
                                           href="<?= BASE_URL ?>estudiante/index?pagina=<?= $i ?>&busqueda=<?= urlencode($busqueda) ?>">
                                            <?= $i ?>
                                        </a>
                                    </li>
                                <?php endfor; ?>
                            <?php else: ?>
                                <!-- Indicador compacto cuando hay muchas paginas -->
